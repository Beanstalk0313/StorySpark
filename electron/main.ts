...
let perchanceView: WebContentsView | null = null;
...
async function importFile(type: string, extensions: string[]) {
...
function updateViewBounds() {
    if (!mainWindow || !perchanceView) return;
...
ipcMain.on('trigger-import', (event, type: string) => {
    let extensions: string[] = [];
    if (type === 'RESEARCH') extensions = ['ssrf'];
    else if (type === 'PERSONA') extensions = ['sspf'];
    else if (type === 'BOOK') extensions = ['ssbf', 'pdf', 'docx', 'txt', 'md'];
    importFile(type, extensions);
});

// Sync Perchance data (LocalStorage + Cookies)
async function getFullPerchanceData() {
    if (!perchanceView) return null;
    try {
...
ipcMain.on('load-perchance-data', async (event, data: any) => {
    if (!perchanceView || !data) return;
    ...
    const injectStorage = () => {
        if (!perchanceView || !data.localStorage) return;
        const script = `
            (function() {
...
ipcMain.on('toggle-perchance-view', (event, visible: boolean, url?: string) => {
    if (!mainWindow) return;
    ...
    if (visible) {
        ...
        if (perchanceView) {
            ...
            (perchanceView as any).webContents.destroy();
            perchanceView = null;
        }
    }
});
